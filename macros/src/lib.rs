...
/// The whole image: the viewBox box the polygons live in, plus the polygons in
/// document (painter's-algorithm) order.
struct OutImage {
    width: f32,
    height: f32,
...
/// recurse; images and text are skipped (outside the supported subset).
fn walk(group: &usvg::Group, out: &mut Vec<OutPoly>) {
    for node in group.children() {
        match node {
            usvg::Node::Group(g) => walk(g, out),
            usvg::Node::Path(p) => emit_path(p, out),
...
/// default fill-then-stroke order, which every supported icon uses).
fn emit_path(path: &usvg::Path, out: &mut Vec<OutPoly>) {
    if !path.is_visible() {
        return;
    }
    ...
    if let Some(fill) = path.fill()
        && let usvg::Paint::Color(c) = fill.paint()
    {
    ...
    if let Some(stroke) = path.stroke()
        && let usvg::Paint::Color(c) = stroke.paint()
    {
        ...
        if !rings.is_empty() {
            out.push(OutPoly {
                ...
                // A stroke outline is always filled nonzero, regardless of the
                // path's own fill-rule.
                even_odd: false,
                rings,
            });
        }
...
/// `const _` registers the source file as a build dependency.
fn emit(image: &OutImage, abs: &str) -> proc_macro2::TokenStream {
