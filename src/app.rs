    ...
    fn handle_main_event(&mut self, event: WindowEvent, event_loop: &ActiveEventLoop) {
        match event {
            ...
            WindowEvent::MouseInput {
                ...
                let ctx = self.dispatch(&event, event_loop);
                // A press / release can change the shape (e.g. grabbing the
                // scrollbar thumb shows the closed hand at once, not just on the
                // first drag motion). Apply it only when a widget actually asked
                // — leaving it untouched otherwise so an ordinary click doesn't
                // flash the hover shape back to the arrow.
                if ctx.cursor_request.is_some()
                    && let Some(win) = self.main_win.clone()
                {
                    reconcile_cursor(&win, &mut self.cursor_icon, ctx.cursor_request);
                }
            }
    ...
    fn handle_popup_event(&mut self, idx: usize, event: WindowEvent, event_loop: &ActiveEventLoop) {
        match event {
            ...
            WindowEvent::MouseInput {
                ...
                let ctx = self.dispatch(&event, event_loop);
                // See the main-window press path: apply a press-driven shape
                // change, but don't reset on a plain click.
                if ctx.cursor_request.is_some()
                    && let Some(p) = self.popups.get_mut(idx)
                {
                    let win = p.win.clone();
                    reconcile_cursor(&win, &mut p.cursor_icon, ctx.cursor_request);
                }
