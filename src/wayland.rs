...
impl State {
    ...
    /// Reconcile the pointer shape after a pointer event: show whatever the
    /// widget under the pointer asked for. The last shape is remembered so an
    /// unchanged hover doesn't re-issue a request. An in-flight *outbound* drag
    /// drives the cursor itself (copy / no-drop), so we stay out of its way
    /// while one is live.
    ///
    /// `reset_if_none` distinguishes a move from a press / release: after a
    /// *move* an unanswered request means "back to the arrow", but after a
    /// press / release it means "leave the shape alone" — otherwise an ordinary
    /// click on a button would flash the hover hand back to the arrow.
    fn apply_cursor(&mut self, requested: Option<Cursor>, reset_if_none: bool) {
        if self.drag_source.is_some() {
            return;
        }
        let want = match requested {
            Some(c) => c,
            None if reset_if_none => Cursor::Default,
            None => return,
        };
        ...
        for event in events {
            ...
            match event.kind {
                PointerEventKind::Enter { .. } | PointerEventKind::Motion { .. } => {
                    ...
                    // rides on. A move resets to the arrow when no one asks.
                    self.apply_cursor(ctx.cursor_request, true);
                    self.mark_popups_dirty();
                }
                ...
                PointerEventKind::Press { button, serial, .. } => {
                    ...
                    let ctx = self.dispatch(Event::PointerDown {
                        pos,
                        button: b,
                        modifiers: self.modifiers,
                    });
                    // Press can change the shape at once (grabbing a thumb), but
                    // a plain click must not reset it — hence `false`.
                    self.apply_cursor(ctx.cursor_request, false);
                    self.mark_popups_dirty();
                }
                PointerEventKind::Release { button, .. } => {
                    ...
                    let ctx = self.dispatch(Event::PointerUp {
                        pos,
                        button: b,
                        modifiers: self.modifiers,
                    });
                    // Release can hand the closed grab back to an open grab over
                    // the thumb; don't reset on an ordinary click.
                    self.apply_cursor(ctx.cursor_request, false);
                    self.mark_popups_dirty();
                }
